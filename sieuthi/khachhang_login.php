...
if (isset($_POST['btn-login'])) {
    ...
    $sql = "SELECT SDT, Gmail, TenTaiKhoan FROM khachhang WHERE (Gmail='$tk' OR SDT='$tk') AND TenTaiKhoan='$mk'";
    $kq = mysqli_query($con, $sql);

    if (mysqli_num_rows($kq) > 0) {
        $_SESSION['username'] = $tk; // Lưu tên người dùng vào session
        header("location: fontend.php");
        exit;
    } else {
    ...
    if ($matkhauMoi === $xacNhanMatKhauMoi) {
        ...
        $sql9 = "UPDATE khachhang SET TenTaiKhoan='$matkhauMoi' WHERE Gmail='$email'";
